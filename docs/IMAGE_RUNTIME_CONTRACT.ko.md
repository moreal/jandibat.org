...
다섯 번째 restore-tools image는 스캔·영수증이 확인된 API/maintenance image ID에서 `/bin/server`, `/bin/maintenance`의 symlink를 실제 실행 파일로 풀어 복사하고, 별도 Nix `restore-tools-busybox` 출력의 정적 BusyBox를 포함합니다. 최종 image에는 API Nix store closure나 `/bin/metrics-proxy`를 복사하지 않습니다. 릴리스 검사는 이 세 실행 파일이 일반 파일이고 `/nix/store`가 없는지 확인합니다.
...
계획된 metrics-only 명령은 API image 안의 별도 executable로 실행하고 API application composition이나 DB 연결을 시작하지 않습니다. `METRICS_SOURCE=api|cockroach`, 내부 바인드 주소 `METRICS_LISTEN_ADDR`, 읽기 전용 Secret 파일 경로 `METRICS_TOKEN_FILE`을 받습니다. 토큰은 파일의 최소 32바이트 무작위 값이며 명령행·로그·image에 넣지 않습니다. API sidecar에는 이 세 설정과 scrape token만 주입하고 고정 `http://127.0.0.1:8080/metrics`를 읽게 합니다. Cockroach 전용 Pod에는 추가로 `COCKROACH_METRICS_HOST`, 읽기 전용 `COCKROACH_METRICS_CA_FILE`, `COCKROACH_METRICS_SERVER_NAME`을 주입하고 정확한 `https://<host>:8080/_status/vars`를 검증된 TLS로 읽게 합니다. 프록시는 caller header를 upstream에 전달하거나 redirect를 따르지 않으며 5초 deadline과 8 MiB 응답 상한을 둡니다. 공개 8080이나 DB·OAuth·SMTP·backup Secret을 프록시에 전달하지 않습니다.

프록시 리스너는 정확한 `GET /metrics`와 `Authorization: Bearer <token>`만 허용합니다. 다른 path는 404, method는 405, 누락·오류 token 또는 forwarding header는 403이며 upstream 요청 전에 거부합니다. 정상은 200 Prometheus text, upstream 오류·redirect·과대 응답·TLS 실패는 502, timeout은 504입니다. 오류 응답과 로그는 token이나 upstream body를 노출하지 않습니다. 이는 별도 인증·NetworkPolicy를 적용한 내부 운영 endpoint이며 GraphQL SDL의 도메인 필드도 공개 OpenAPI edge endpoint도 아닙니다.

뒤따르는 native backup verifier는 read-only 권한으로 파일 검사한 `collectionId`, full·incremental `linkedScheduleIds`, `chainId`, `checkedAt`, `verifiedRecoveryAt`, `outcome`만 안전한 JSON 기록 `/var/run/jandibat-backup/verified.json`에 원자적으로 저장합니다. 검증기의 인증된 내부 `GET /metrics`는 다른 path에 404, method에 405, 누락·오류 Bearer token에 403을 반환하고 검사 전에 거부합니다. 정상 scrape는 검사 실패·stale 상태에도 200으로 답하면서 건강 metric을 0/unknown으로 표시하고 검증된 복구 시각을 전진시키지 않습니다. 보존 계획은 `bucket`, `prefix`, `chainRange`, `targets`의 정확한 `objectKey`·`versionId`·`expectedBytes`, `catalogDigest`, `coverage`, `expiresAt`, `approvalHash`를 기록하며 credential이나 서명 URL은 포함하지 않습니다. 구현 소유권과 필드 의미는 [인터페이스 변경 로그](interface-change-log.md)의 2026-09-25 운영 계약을 따릅니다.

Migration Job은 application rollout보다 먼저 실행합니다. Job image에는 Cockroach `cockroach sql` CLI, `sh`, checksum 도구, 읽기 전용 migration SQL/script가 있어야 하며 임시 transaction file용 writable `/tmp`가 있어야 합니다. `MIGRATION_DATABASE_URL`, `COCKROACH_DATABASE`, `MIGRATIONS_DIR`을 명시하고 URL의 database와 이름이 일치하는지 runner가 확인합니다. Migration, role 설정, role 검증 스크립트는 각 `cockroach sql` 자식 프로세스에 DB URL을 `COCKROACH_URL` 환경 변수로 전달하며 URL을 명령행 인자로 넣지 않습니다. Migration 적용 후 같은 credential로 `scripts/db-configure-runtime-roles.sh`를 실행하여 세 runtime role의 GRANT를 갱신합니다. 그 성공과 role negative check를 확인한 뒤 API/worker/maintenance를 시작합니다. 같은 migration을 재실행하면 version/checksum이 일치하는 파일은 `already applied`로 처리하고 checksum 변경은 실패합니다. 실패한 Job은 애플리케이션 rollout을 막습니다.
