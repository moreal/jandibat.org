...
호스트 계약 확인은 `nix develop --command sh scripts/test-image-runtime-contract.sh`, Linux image/container 확인은 `nix develop --command make images-smoke`를 사용합니다. Darwin에서 image output 평가와 Go process 테스트가 통과해도 Linux container smoke를 수행했다고 기록하지 않습니다.
